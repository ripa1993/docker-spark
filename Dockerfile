...
# MAVEN 3.3.3
RUN wget http://mirror.olnevhost.net/pub/apache/maven/maven-3/3.3.3/binaries/apache-maven-3.3.3-bin.tar.gz && sudo tar -zxf apache-maven-3.3.3-bin.tar.gz -C /usr/local/ && rm apache-maven-3.3.3-bin.tar.gz && \
...
# Clean
RUN rm -rf /root/.m2/repository && rm -rf /usr/local/apache-maven-3.3.3 && find /usr/share/doc -depth -type f ! -name copyright|xargs rm || true && find /usr/share/doc -empty|xargs rmdir || true && rm -rf /usr/share/man/* /usr/share/groff/* /usr/share/info/* && rm -rf /usr/share/lintian/* /usr/share/linda/* /var/cache/man/*
